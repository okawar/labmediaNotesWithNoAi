...
function saveNote() {
  ...
  } else if (noteToEdit.value) {
    notesStore.editNote(
      noteToEdit.value.number,
      {
        title: title.value,
        content: content.value,
        imgSrc: imgSrc.value || undefined
    ...
    <div class="modal-backdrop" v-show="isModalOpen">
        <div class="modal">
            ...
            <form action="" class="modal-form" @submit.prevent="saveNote">
                ...
                <textarea v-model="content" placeholder="Текст заметки" class="modal-textarea-content" :class="{'with-image': imgSrc}"></textarea>
...
<style scoped>
    ...
    .modal-textarea-content.with-image {
        min-height: 240px;
    }
    ...
    @media (max-width: 375px) {
        ...
        .modal-input-title, .modal-textarea-content {
            width: 90%;
            margin-left: -5px;
        }
